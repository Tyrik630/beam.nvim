...
local test_cases = {
  -- Quotes
  {
    ...
    objects = {
      ...
      ['a"'] = { pos = {1, 5}, yank_expect_any = {'"bar"', '"bar" '}, delete_result_any = {'foo  baz', 'foo baz'} }
    }
  },
  {
    name = "single quotes",
  ...
  },
  {
    ...
    objects = {
      ...
      ['aW'] = { pos = {1, 4}, yank_expect_any = {'foo-bar ', ' foo-bar'}, delete_result = 'foo baz' }
    }
  },
  ...
  -- Sentence (basic test)
  {
    ...
    objects = {
      ['is'] = { pos = {1, 16}, yank_expect = 'Second sentence.', delete_result_match = 'First sentence.*Third' },
      ['as'] = { pos = {1, 16}, yank_expect_match = {'Second sentence'}, delete_result_match = 'First sentence.*Third' }
    }
  },
  ...
  -- HTML/XML tags (if supported)
  {
    ...
    objects = {
      ['it'] = { pos = {1, 5}, yank_expect = 'content', delete_result = '<div></div>', skip_if_fails = true },
      ['at'] = { pos = {1, 5}, yank_expect = '<div>content</div>', delete_result = '', skip_if_fails = true }
    }
  }
}
      ...
      test(test_name, function()
        ...
        if operation == 'yank' then
          ...
          if expected.yank_expect then
            assert(yanked == expected.yank_expect, 
          ...
          elseif expected.yank_expect_any then
            ...
            for _, exp in ipairs(expected.yank_expect_any) do
              if yanked == exp then found = true; break end
            end
            assert(found, string.format("Expected one of %s, got '%s'", 
              vim.inspect(expected.yank_expect_any), yanked))
          elseif expected.yank_expect_match then
            for _, pattern in ipairs(expected.yank_expect_match) do
              assert(yanked:match(pattern), 
        ...
        elseif operation == 'delete' then
          if expected.delete_result then
            ...
            assert(result == expected.delete_result, 
          ...
          elseif expected.delete_result_any then
            ...
            for _, exp in ipairs(expected.delete_result_any) do
              if result == exp then found = true; break end
            end
            assert(found, string.format("Expected one of %s, got '%s'", 
              vim.inspect(expected.delete_result_any), result))
          elseif expected.delete_result_match then
            local result = table.concat(vim.api.nvim_buf_get_lines(0, 0, -1, false), ' ')
            assert(result:match(expected.delete_result_match), 
          ...
          elseif expected.delete_lines_expect then
            ...
            assert(#lines == expected.delete_lines_expect, 
